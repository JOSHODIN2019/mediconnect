...
export default function PatientChat() {
  ...
  // Keep ref in sync so socket handlers can read latest partnerId without stale closure
  ...
  // Socket: join room + listen for incoming messages + read receipts
  useEffect(() => {
    if (!socket) return
    ...
    const onMessage = (msg) => {
      setMessages(prev => {
        // Deduplicate — sender gets an echo from the room AND personal room emit
        if (prev.some(m => m._id === msg._id)) return prev

        // Only show messages that belong to the currently open conversation.
        // Use sender/receiver IDs directly — more reliable than room-string comparison
        // since activeRoom.current may not be set yet when the event fires.
    ...
    // ── messages_read — other party opened & read our messages ──
    const onRead = ({ room }) => {
  ...
  // Join/leave room when partnerId or socket changes
  useEffect(() => {
    ...
    return () => {
      // leave handled by server on disconnect / next join
    }
    ...
    <div className="flex h-[calc(100vh-56px)] overflow-hidden bg-neutral-50">
      ...
      <div className={[
        ...
        <div className="px-4 py-4 border-b border-neutral-100">
          ...
          <p className="text-xs text-neutral-500 mt-0.5">Chat with your doctors</p>
        </div>
        ...
        {loadingConvs ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center px-6 text-center gap-3">
            ...
            <p className="text-xs text-neutral-400 leading-relaxed">
              Book an appointment with a doctor and start a conversation.
            </p>
          </div>
        ) : (
          <div className="flex-1 overflow-y-auto">
            {conversations.map(conv => (
              <button
                ...
                onClick={() => navigate(`/patient/chat/${conv.partner._id}`)}
                className={[
      ...
      {!partnerId ? (
        <div className="hidden sm:flex flex-1 items-center justify-center">
          <div className="text-center space-y-3">
            ...
            <p className="text-sm text-neutral-400">Choose a doctor from the list to start chatting.</p>
          </div>
        </div>
      ) : (
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex items-center gap-3 px-4 py-3.5 bg-white border-b border-neutral-200 flex-shrink-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-neutral-400">{partner?.specialization || 'Doctor'}</p>
            </div>
          </div>
          ...
          <div className="flex items-end gap-2 px-4 py-3 bg-white border-t border-neutral-200 flex-shrink-0">
            <textarea
              ...
              rows={1}
              className="flex-1 resize-none px-3.5 py-2.5 text-sm bg-neutral-50 border border-neutral-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-colors max-h-28 overflow-y-auto"
              style={{ lineHeight: '1.5' }}
            />
            <button
              onClick={send}
              disabled={!text.trim() || !socket}
              className="w-10 h-10 rounded-xl bg-blue-600 hover:bg-blue-700 disabled:bg-neutral-200 disabled:cursor-not-allowed flex items-center justify-center transition-colors flex-shrink-0"
            >
              <SendIcon />
            </button>
          </div>
...
function ReadTick({ isRead }) {
  return isRead ? (
    <svg width="14" height="10" viewBox="0 0 14 10" fill="none" className="flex-shrink-0">
      <path d="M1 5l3 3 5-6" stroke="#2563eb" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
      <path d="M5 5l3 3 5-6" stroke="#2563eb" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  ) : (
...
function ChatEmptyIcon({ small }) {
  const s = small ? 20 : 26
  return <svg width={s} height={s} viewBox="0 0 26 26" fill="none" stroke="#93c5fd" strokeWidth="1.5" strokeLinecap="round"><path d="M22 4H4a2 2 0 00-2 2v12a2 2 0 002 2h14l4 4V6a2 2 0 00-2-2z"/><path d="M8 10h10M8 14h6"/></svg>
}
function SendIcon() {
